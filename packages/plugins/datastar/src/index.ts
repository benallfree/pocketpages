...
import { RealtimeOptions } from 'pocketpages-plugin-realtime'
import loader from './loader.html'
import realtime from './realtime.html'
...
const datastarPluginFactory: PluginFactory = (config) => {
  ...
  return {
    ...
    onExtendContextApi: ({ api }) => {
      ...
      api.datastar = {
        scripts(options?: Partial<ScriptOptions>) {
          const scripts = [loader]
          if (options?.spa) {
            scripts.push(
              spa
                .replaceAll('{{scope}}', options.spa.scope)
                .replaceAll('{{selector}}', options.spa.selector || '')
            )
          }
          if (options?.realtime) {
            scripts.push(realtime)
          }
          return scripts.join('\n')
        },
        realtime: {
          ...
          patchElements: (
            elements: string,
            patchOptions?: Partial<PatchElementsOptions>,
          ...
          const script = `
{
  "prefetch": [
    ...
    onRender: ({ content, api }) => {
      if (api.request.header('Datastar-Request') !== 'true') {
        return content
      }
      const selector = api.request.header('Datastar-Selector')
      const options: Partial<PatchElementsOptions> = {}
      if (selector) {
        // @ts-ignore
      ...
      // @ts-ignore
      api.datastar.patchElements(content, options)
